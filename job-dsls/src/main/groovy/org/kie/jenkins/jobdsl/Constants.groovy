/*
 * Copyright 2017 Red Hat, Inc. and/or its affiliates.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package org.kie.jenkins.jobdsl


class Constants {
    static final String MAVEN_VERSION = "3.5.4"
    static final String UPSTREAM_BUILD_MAVEN_VERSION = "3.5.4"
    static final String JDK_VERSION = "kie-jdk1.8"
    static final String BRANCH = "master"
    static final String GITHUB_ORG_UNIT = "kiegroup"
    static final String PULL_REQUEST_FOLDER = "pullrequest"
    static final String DEPLOY_FOLDER = "deployedRep"
    static final String ZANATA_VERSION = "7.18.0"
    static final String ERRAI_BRANCH = "master"
    static final String ERRAI_SNAPSHOT = "4.5.0-SNAPSHOT"
    static final String ERRAI_PREFIX = "4.5.0"
    static final String KIE_PREFIX = "7.18.0"
    static final String KIE_PROD_BRANCH_PREFIX = "7.18.x"
    static final String UBERFIRE_PREFIX = "2.15.0"

}
