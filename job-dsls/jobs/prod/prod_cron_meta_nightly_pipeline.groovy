...
def SERVERLESS_LOGIC_CURRENT_PRODUCT_VERSION='991.39.0'
def SERVERLESS_LOGIC_KOGITO_CURRENT_PRODUCT_VERSION='999.106.0'
def SERVERLESS_LOGIC_DROOLS_CURRENT_PRODUCT_VERSION='999.106.0'
def SERVERLESS_LOGIC_CURRENT_PRODUCT_BRANCH='999.106.x-prod'
def SERVERLESS_LOGIC_CURRENT_PRODUCT_CONFIG_BRANCH="openshift-serverless-logic/999.106.x"
        ...
        stage('trigger Drools Ansible Integration nightly job ${branch}') {
            steps {
                build job: 'drools-ansible-integration.nightly/${branch}', propagate: false, wait: true, parameters: [
                        [\$class: 'StringParameterValue', name: 'NEXUS_DEPLOYMENT_REPO_URL', value: "\${env.BXMS_QE_NEXUS}/service/local/repositories/scratch-release-drools-ansible-integration-${getNexusFromVersion(version)}/content-compressed"],
